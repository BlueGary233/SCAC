...
2. **调整限制器**：
   - 最小白位限制：防止阴暗画面过亮（默认0.85，建议根据画面调整）
   - 最大黑位限制：避免将灰色校准成黑色（默认0.15，建议根据画面调整）
...
#### 高级设置
- **采样区域**：选择全屏或自定义区域采样
- **平滑度控制**：调整平滑度参数，值越大变化越平滑
- **调试视图**：启用后显示所有规约纹理和历史数据

### 技术细节

#### 工作原理
1. **亮度计算**：使用两个独立的函数计算像素亮度
   - **最小亮度**：使用 `min(min(r,g),b)` 公式计算像素的最小亮度值
   - **最大亮度**：使用 `max(max(r,g),b)` 公式计算像素的最大亮度值
2. **多级规约**：通过5次4x4规约将1024x1024采样降至1x1统计
3. **单帧平滑**：使用平滑算法避免亮度值突变，提供稳定的颜色校准
...
#### 纹理用途
- **TextureMip0-4**：多级规约纹理（1024x1024 → 4x4）
- **TextureMip5**：最终统计纹理（1x1），存储当前帧亮度极值和平滑后的历史值
...
#### UI过滤
- **开启UI过滤**：启用/禁用UI过滤（默认启用）
- **显示过滤效果**：显示被滤除的像素（红色=过亮，蓝色=过暗）
- **ui暗色过滤**：滤除过暗UI像素的阈值（默认0.00001）
- **ui亮色过滤**：滤除过亮UI像素的阈值（默认100.0）
...
2. **Adjust Limiters**:
   - Min White Limiter: Prevents overly bright dim scenes (default: 0.85, adjust based on scene)
   - Max Black Limiter: Avoids calibrating grey as black (default: 0.15, adjust based on scene)
...
#### How It Works
1. **Luminance Calculation**: Uses two independent functions to calculate pixel luminance
   - **Minimum Luminance**: Uses `min(min(r,g),b)` formula to calculate the minimum luminance value of a pixel
   - **Maximum Luminance**: Uses `max(max(r,g),b)` formula to calculate the maximum luminance value of a pixel
...
#### UI Filtering
- **Enable UI Filter**: Enable/disable UI filtering (enabled by default)
- **Show Filter Effect**: Display filtered pixels (red=too bright, blue=too dark)
- **UI Filter Black Level**: Threshold for filtering overly dark UI pixels (default: 0.00001)
- **UI Filter White Level**: Threshold for filtering overly light UI pixels (default: 100.0)
...
#### v0.0.10 (2025/12/22)
- Improved brightness calculation algorithm, maximum and minimum brightness are obtained separately
- Optimized performance

#### v0.09 (2025/12/21)
- Modified some default values and UI layout
- Improved parameter adjustment ranges
